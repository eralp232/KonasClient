...
public class CrystalUtils {
    ...
    public static boolean rayTracePlace(BlockPos pos) {
        if (AutoCrystal.directionMode.getValue() != AutoCrystal.DirectionMode.VANILLA) {
                ...
                for (double yS = start; yS <= end; yS += increment) {
                    for (double zS = start; zS <= end; zS += increment) {
                        ...
                        if (AutoCrystal.strictDirection.getValue()) {
                            if (distToPosVec > AutoCrystal.placeRange.getValue()) continue;
                        }
            ...
            for (EnumFacing facing : EnumFacing.values()) {
                ...
                if (AutoCrystal.strictDirection.getValue()) {
                    if (mc.player.getPositionVector().add(0, mc.player.getEyeHeight(), 0).distanceTo(cVector) > AutoCrystal.placeRange.getValue()) {
                        continue;
                    }
                }
    ...
    public static float calculateDamage(double posX, double posY, double posZ, Entity entity) {
        ...
        Vec3d entityPosVec = getEntityPosVec(entity, AutoCrystal.predictTicks.getValue() > 0 ? AutoCrystal.predictTicks.getValue() : 0);
        ...
        try {
            if (AutoCrystal.terrainIgnore.getValue()) {
                blockDensity = getBlockDensity(vec3d, AutoCrystal.predictTicks.getValue() > 0 ?
                        entity.getEntityBoundingBox().offset(getMotionVec(entity, AutoCrystal.predictTicks.getValue())) :
                        entity.getEntityBoundingBox());
            } else {
                blockDensity = entity.world.getBlockDensity(vec3d, AutoCrystal.predictTicks.getValue() > 0 ?
                        entity.getEntityBoundingBox().offset(getMotionVec(entity, AutoCrystal.predictTicks.getValue())) :
                        entity.getEntityBoundingBox());
            }
        } catch (Exception ignored) {
    ...
    public static int getCrystalSlot() {
        ...
        if (Module.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL) {
            crystalSlot = Module.mc.player.inventory.currentItem;
        }


        if (crystalSlot == -1) {
            for (int l = 0; l < 9; ++l) {
                if (Module.mc.player.inventory.getStackInSlot(l).getItem() == Items.END_CRYSTAL) {
                    crystalSlot = l;
                    break;
                }
            }
    ...
    public static int getSwordSlot() {
        ...
        if (Module.mc.player.getHeldItemMainhand().getItem() == Items.DIAMOND_SWORD) {
            swordSlot = Module.mc.player.inventory.currentItem;
        }

        if (swordSlot == -1) {
            for (int l = 0; l < 9; ++l) {
                if (Module.mc.player.inventory.getStackInSlot(l).getItem() == Items.DIAMOND_SWORD) {
                    swordSlot = l;
                    break;
                }
            }
