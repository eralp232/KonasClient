...
import me.darki.konas.module.modules.client.RenderModule;
import me.darki.konas.module.modules.render.Interactions;
...
public class BlockRenderUtil {
    ...
    public static void renderPlacementPos(BlockPos pos) {
        if (Module.mc.world == null) return;

        if (!ModuleManager.getModuleByClass(Interactions.class).isEnabled()) return;

        AxisAlignedBB bb = Module.mc.world.getBlockState(pos).getBoundingBox(Module.mc.world, pos).offset(pos);

        ((Interactions) ModuleManager.getModuleByClass(Interactions.class)).renderPlacingBB(bb);
    }
}
