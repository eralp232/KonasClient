...
import me.darki.konas.event.events.BlockRenderEvent;
...
public class Search extends Module {
    ...
    private static Setting<Float> range = new Setting<>("Range", 100f, 500f, 1f, 1f);
    private static Setting<ColorSetting> color = new Setting<>("Color", new ColorSetting(0xFF00FFFF));

    private static Setting<Boolean> defaultSetting = new Setting<>("Default", true);
    private static Setting<Boolean> custom = new Setting<>("Custom", true);
    private static Setting<Boolean> illegals = new Setting<>("Illegals", true);

    private static Setting<Boolean> tracers = new Setting<>("Tracers", false);
    private static Setting<Boolean> fill = new Setting<>("Fill", true);
    private static Setting<Boolean> outline = new Setting<>("Outline", true);

    public static Setting<Boolean> softReload = new Setting<>("SoftReload", true);
    private static Setting<Boolean> slowRender = new Setting<>("SlowRender", false);
    ...
    public void onEnable() {
        if (GlStateManager.glGetString(GL_VENDOR).contains("Intel") && !slowRender.getValue()) {
            Logger.sendChatMessage("You have an integrated graphics card. To increase fps, use SlowRender.");
        }
    ...
    private boolean shouldAdd(Block block, BlockPos pos) {
        ...
        if (custom.getValue()) {
            if (customBlocks.getValue().getBlocks().contains(block)) {
                return true;
            }
        }

        if (illegals.getValue()) {
            if (isIllegal(block, pos)) {
                return true;
            }
        }
    ...
    private boolean shouldRender(BlockVec vec) {
        ...
        if (custom.getValue()) {
            if (customBlocks.getValue().getBlocks().contains(mc.world.getBlockState(new BlockPos(vec.x, vec.y, vec.z)).getBlock())) {
                return true;
            }
        }

        if (illegals.getValue()) {
            if (isIllegal(mc.world.getBlockState(new BlockPos(vec.x, vec.y, vec.z)).getBlock(), new BlockPos(vec.x, vec.y, vec.z))) {
                return true;
            }
        }
    ...
    private boolean isIllegal(Block block, BlockPos pos) {
        ...
        if (block == Blocks.BEDROCK) {
            if (mc.player.dimension == 0) {
                ...
                if (pos.getY() > 127 || (pos.getY() < 123 && pos.getY() > 4)) return true;
            } else {
                return true;
            }
        }
