...
import me.darki.konas.module.modules.combat.AutoTrap;
...
public class InteractionUtil {
    ...
    public static boolean canPlaceNormally() {
        return !KonasGlobals.INSTANCE.rotationManager.isRotationsSet();
    }
    ...
    public static Placement preparePlacement(BlockPos pos, boolean rotate, boolean instant, boolean strictDirection, boolean rayTrace) {
        ...
        float[] angle = RotationManager.calculateAngle(mc.player.getPositionEyes(mc.getRenderPartialTicks()), hitVec);
        if (rotate) {
            if (instant) {
    ...
    public static boolean canClick(BlockPos pos) {
        return mc.world.getBlockState(pos).getBlock().canCollideCheck(mc.world.getBlockState(pos), false) || AutoTrap.shiftedBlocks.containsKey(pos) ;
    }
    ...
    public static List<EnumFacing> getPlacableFacings(BlockPos pos, boolean strictDirection, boolean rayTrace) {
        ...
        for (EnumFacing side : EnumFacing.values()) {
            ...
            IBlockState blockState = mc.world.getBlockState(neighbour);
            if ((blockState == null || !blockState.getBlock().canCollideCheck(blockState, false) || blockState.getMaterial().isReplaceable()) && !AutoTrap.shiftedBlocks.containsKey(neighbour)) continue;
